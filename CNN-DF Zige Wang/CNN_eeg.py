...
EARLY_STOP_PARAM = 10
CONDITION = 2
SUB_LIST1 = [2, 3, 4, 6, 8, 9, 11, 13, 14, 16, 19, 20]
SUB_LIST2 = [2, 3, 4, 6, 8, 9, 11, 13, 14, 16, 19, 20]
...
def main():
    ...
    x = tf.placeholder(tf.float32, [None, 128*180])
    ...
    x_eeg = tf.reshape(x, [-1, 180, 128, 1])
    ...
    w_conv2 = weight_variable([6, 1, 96, 128])  # 多通道卷积，卷积出128个特征
    ...
    w_conv3 = weight_variable([6, 1, 128, 128])  # 多通道卷积，卷积出128个特征
    ...
    w_fc1 = weight_variable([38 * 128, 2048])
    b_fc1 = bias_variable([2048])

    h_conv3_flat = tf.reshape(h_conv3, [-1, 38 * 128])  # 展开，第一个参数为样本数量，-1未知
    ...
    correct_set = np.load("experiments_insub/CNN/CNN_100_INSUB_CON" + str(CONDITION) + ".npy")
    ...
    for s in SUB_LIST:
        ...
        for n in range(REPEAT_TIME):
            ...
            if EARLY_STOP:
                ...
                save_file = "experiments_insub/CNN/models/insub_con" + str(CONDITION) + ".ckpt"
            with tf.Session() as sess:
                ...
                toc = time.time()
                computing = toc - tic
                print("Sub %d Con %d Repeat %d: Accuracy on test data: %g Using %g seconds" % (s, CONDITION, (n + 1), test_accuracy, computing))
                total += computing
                correct_set[s - 1, n] = test_accuracy

            if n % 50 == 0:
                np.save("experiments_insub/CNN/CNN_100_INSUB_INTERP_CON" + str(CONDITION) + ".npy", correct_set)

    average_time = total/REPEAT_TIME
    print("Average computing time %g" % average_time)
    np.save("experiments_insub/CNN/CNN_100_INSUB_INTERP_CON" + str(CONDITION) + ".npy", correct_set)
